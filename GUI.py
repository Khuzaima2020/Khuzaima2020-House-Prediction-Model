...
class MainPage(tk.Frame):
    def __init__(self, master, app):
        ...
        self.app = app
        label = ttk.Label(self, text="Welcome to Price Predictor")
        label.pack(pady=20)
        predict_button = ttk.Button(self, text="Predict Price", command=self.navigate_to_input_page)
...
class InputPage(tk.Frame):
    def __init__(self, master, app):
        ...
        # Set column weight to make them expand
        ...
        predict_button = ttk.Button(self, text="Predict Price", command=self.predict_price)
        predict_button.grid(row=1, column=0, columnspan=2, pady=10)

        back_button = ttk.Button(self, text="Back to Main Page", command=self.navigate_to_main_page)
    ...
    def predict_price(self):
        try:
            ...
            result_label = ttk.Label(self, text=f'Predicted Price: {predicted_price[0]:.2f}')
            result_label.grid(row=3, column=0, columnspan=2, pady=10)
        except Exception as e:
            result_label = ttk.Label(self, text=f'Error: {str(e)}')
            result_label.grid(row=3, column=0, columnspan=2, pady=10)
